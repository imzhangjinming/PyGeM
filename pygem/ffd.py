...
from pygem.utils import fit_affine_transformation, angles2matrix, angles2matrix2d, phy2ffd_map2d, ffd2phy_map2d
...
class FFD2D(Deformation):
    """
    Class that handles the Free Form Deformation on the 2d mesh points.

    :param list n_control_points: number of control points in the x and y
        direction. Default is [2, 2].
        
    :cvar numpy.ndarray box_length: dimension of the FFD bounding box, in the
        x and y direction (local coordinate system).
    :cvar numpy.ndarray box_origin: the x and y coordinates of the origin of
        the FFD bounding box.
    :cvar numpy.ndarray rot_angle: rotation angle around x, y and z axis of the
        FFD bounding box.
    :cvar numpy.ndarray n_control_points: the number of control points in the
    ...
    :Example:
        ...
        >>> new_mesh_points = ffd(original_mesh_points)
    """
    reference_frame = np.array([[0, 0], [1, 0], [0, 1]])

    def __init__(self, n_control_points=None):
        self.conversion_unit = 1. # TODO: unused at the moment

        self.box_length = np.array([1., 1.])
        self.box_origin = np.array([0., 0.])
        ...
        if n_control_points is None:
            n_control_points = [2, 2]
        self.n_control_points = n_control_points

    @property
    def n_control_points(self):
        """
        The number of control points in X and Y directions
    ...
    def T(self):
        """
        Return the function that deforms the points within the unit rectangle.

        :rtype: callable
        """
        def T_mapping(points):
            (n_rows, n_cols) = points.shape
            (dim_n_mu, dim_m_mu) = self.array_mu_x.shape
            ...
            for j in range(0, dim_m_mu):
                ...
                for i in range(0, dim_n_mu):
                    ...
                    aux_x += aux * self.array_mu_x[i, j]
                    aux_y += aux * self.array_mu_y[i, j]
                    # aux_z += aux * self.array_mu_z[i, j, k]
    ...
    def rotation_matrix(self):
        """
        The rotation matrix (according to rot_angle).

        :rtype: numpy.ndarray
        """
        return angles2matrix2d(np.radians(self.rot_angle[0]))
    ...
    def read_parameters(self, filename='parameters.prm'):
        ...
        for line in mux.split('\n'):
            values = np.array(line.split())
            self.array_mu_x[tuple(map(int, values[0:2]))] = float(values[2])

        for line in muy.split('\n'):
            values = line.split()
            self.array_mu_y[tuple(map(int, values[0:2]))] = float(values[2])
        ...
        # self.n_control_points[0] = config.getint('Box info',
        #                                          'n control points')
        # self.ctrl_points = np.zeros((self.n_control_points[0], 2))
        
        # control_points = config.get('Box info', 'control points')
        # for idx, line in enumerate(control_points.split('\n')):
        #     values = np.array(line.split())
        #     self.ctrl_points[idx, :] = values
    ...
    def control_points(self, deformed=True):
        ...
        y_coords, x_coords = np.meshgrid(y, x)
    ...
    def __call__(self, src_pts):
        ...
        :param numpy.ndarray src_pts: the array of dimensions (*n_points*, *2*)
            containing the points to deform. The points have to be arranged by
            row.
        :return: the deformed points
        :rtype: numpy.ndarray (with shape = (*n_points*, *2*))
        """
        def is_inside(pts, boundaries):
            """ 
            Check is `pts` is inside the ranges provided by `boundaries`.
...
class FFD2D_irregular(Deformation):
    """
    Class that handles the Free Form Deformation on the 2d mesh points with irregular control points.
    ...
    :Example:
        ...
        >>> from pygem import FFD2D_irregular
        >>> import numpy as np
        >>> ffd = FFD()
        >>> ffd.read_parameters(
        >>>        'tests/test_datasets/parameters_test_ffd_sphere.prm')
    ...
    def read_parameters(self, filename='parameters.prm'):
        ...
        # self.n_control_points[1] = config.getint('Box info',
        #                                          'n control points y')
        self.ctrl_points = np.zeros((self.n_control_points[0]*self.n_control_points[1], 2))
    ...
    def control_points(self, deformed=True):
        ...
        box_points = np.zeros_like(self.ctrl_points)

        if deformed:
            box_points += self.ctrl_points + np.hstack((self.array_mu_x.reshape((-1,1)), self.array_mu_y.reshape((-1,1))))
        else:
            box_points += self.ctrl_points
        # n_rows = box_points.shape[1]
    ...
    def __call__(self, src_pts):
        ...
        m, n  = self.array_mu_x.shape[0], self.array_mu_x.shape[1]
        # get uv coordinates
        self.uv = phy2ffd_map2d(self.control_points(deformed=False), src_pts, m, n)
        dst_pts = ffd2phy_map2d(self.control_points(deformed=True), self.uv, dim_x=m, dim_y=n)
